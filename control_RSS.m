function [new_state_dot, velocity] = control_RSS(v_ref, path ,step, state_dot, state)
    ...
    for m = 1 : max_iter
        ...
        % 先删除旧日志（避免残留）
        if exist(log_file, 'file'), delete(log_file); 
        end
        diary(log_file); 
        diary on;
        ...
        cvx_begin % 无quiet、无cvx_options，纯低版本语法
            ...
            J = 0 ;
            for k = 2: K
                J = J + 30* sum_square( nu(1: 2, k) - [1 ; 1]);
            end

            for k = 1: K
                J = J + k1 * sum_square(psi(k) - 1);
            end

            minimize(J + sum_square(u(:)) + rho * sum_square(u(:) - u_hat(:)));
